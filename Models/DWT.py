...
class DWT(tf.keras.layers.Layer):
    ...
    def __init__(self):
        super().__init__()
        ########
        self.p_weight = tf.constant([1], dtype=tf.float32)[None, None, :]
        #self.register_buffer('p_weight', tf.constant(p_weight, dtype=float32)[None, None, :])
        self.p_params = dict(bias=False, strides=1, dilation_rate=1, groups=1)
        self.p_padding = (self.p_weight.shape[2]-1)//2
        #
        self.u_weight = tf.constant([0.5], dtype=tf.float32)[None, None, :]
        ...
        #
        self.scaling_factor = tf.constant(numpy.sqrt(2.0), dtype=tf.float32)
