...
class DWT(tf.keras.layers.Layer):
    ...
    def __init__(self):
        ...
        self.register_buffer('p_weight', tf.constant(
            p_weight, dtype=tf.float32)[None, None, :])
        self.p_params = dict(bias=False, strides=1, dilation_rate=1, groups=1)
        self.p_padding = (self.p_weight.shape[2]-1)//2
        #
        u_weight = [0.5]
        self.register_buffer('u_weight', tf.constant(u_weight, dtype=tf.float32)[None, None, :])
        ...
        self.register_buffer('scaling_factor', tf.constant(scaling_factor, dtype=tf.float32))
